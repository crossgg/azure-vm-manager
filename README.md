...
账号绑定按 `provider/account` 精确匹配。绑定后，该账号的令牌、实例列表与详情、账单/监控、状态操作、换 IP、OCI 编辑与安全规则等所有官方云 API 请求均使用主代理。fallback 可选择 `None`（不回退）、直连，或池内另一条代理；主代理失败时仅在配置了 fallback 时尝试后续路由。
